...
.addBatch({
    'when paginating TestEntry querying for all documents, with page 1, 10 results per page': {
        ...
        'there should be no errors': function(error, page) {
            assert.equal(error, null);
        },
        'results.length should be 10': function(error, page) {
            assert.equal(page.results.length, 10);
        },
        'the first result should contain the correct index #(1)': function(error, page) {
            assert.equal(page.results[0].title, 'Item #1');
        }
    }
})
...
.addBatch({
    'when paginating TestEntry querying for all documents, with page 2, 10 results per page': {
        ...
        'the first result should contain the correct index #(11)': function(error, page) {
            assert.equal(page.results[0].title, 'Item #11');
        },
        'there should be 100 items as results': function(error, page) {
...
.addBatch({
    'when paginating TestEntry querying for all documents, with page 10, 11 results per page': {
        ...
        'the first result should contain the correct index #(100)': function(error, page) {
            assert.equal(page.results[9].title, 'Item #100');
        }
    }
})
...
.addBatch({
    'when paginating TestEntry querying for all documents, with page 2, 10 results per page with populate and without columns': {
        ...
        'the first result should contain the correct SubItem #(1)': function(error, page) {
            assert.equal(page.results[0].child.title, 'SubItem #1');
        }
    }
})
...
.addBatch({
    'when paginating TestEntry querying for all documents, with page 1, 10 results per page, sorting reverse by title': {
        ...
        'the first result should contain the correct index #(99)': function(error, page) {
            assert.equal(page.results[0].title, 'Item #99');
        }
    }
})
